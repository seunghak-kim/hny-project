...
def chunk_realtor_law_to_json(
    input_path="공인중개사법(법률)(제19841호)(20241227).txt",
    output_path=None
):
    """
    공인중개사법을 조 단위로 청킹하고 JSON 파일로 저장합니다.
    특징: 7개 장, 75개 조항, 주요 모법
    """
    ...
    # 제목 및 메타데이터 추출
    title_match = re.search(r'^([^\n]*공인중개사법)', content, re.MULTILINE)
    law_title = title_match.group(1).strip() if title_match else "공인중개사법"
    ...
    for i, match in enumerate(article_matches):
        ...
        # 개정/신설/전문개정 정보 추출
        amendments = re.findall(r'<(개정|신설|전문개정)\s+[^>]+>', article_content)
        newly_established = re.search(r'\[본조신설\s+([^\]]+)\]', article_content)

        # 시행일 특별 규정 확인
        special_enforcement = re.search(r'\[시행일\s*:\s*([^\]]+)\]', article_content)
        ...
        # 개정 정보가 있으면 메타데이터에 추가
        if amendments:
            metadata["has_amendments"] = True
            metadata["amendment_count"] = len(amendments)

        # 본조신설 정보가 있으면 메타데이터에 추가
        if newly_established:
            metadata["newly_established"] = newly_established.group(1)

        # 특별 시행일이 있으면 메타데이터에 추가
        if special_enforcement:
            metadata["special_enforcement_date"] = special_enforcement.group(1)
    ...
    try:
        ...
        for chunk in chunks:
            ...
            if chunk['metadata'].get('has_amendments'):
                amended_count += 1
            if chunk['metadata'].get('newly_established'):
                newly_established_count += 1
        ...
        print(f"\n개정 통계:")
        print(f"  개정된 조항: {amended_count}개")
        print(f"  신설 조항: {newly_established_count}개")
