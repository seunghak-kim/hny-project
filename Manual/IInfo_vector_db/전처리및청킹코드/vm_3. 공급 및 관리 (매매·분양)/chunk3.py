...
def chunk_building_sales_law_to_json(
    input_path="건축물의 분양에 관한 법률(법률)(제17007호)(20210101).txt",
    output_path=None
):
    """
    건축물의 분양에 관한 법률을 조 단위로 청킹하고 JSON 파일로 저장합니다.
    특징: 장(Chapter) 구분 없음, 21개 조항
    """
    ...
    # 제목 및 메타데이터 추출
    title_match = re.search(r'^([^\n]+법률?)\s*\[', content, re.MULTILINE)
    law_title = title_match.group(1).strip() if title_match else "건축물의 분양에 관한 법률"
...
if __name__ == "__main__":
    chunk_building_sales_law_to_json()