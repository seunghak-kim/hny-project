...
def chunk_apartment_pricing_rule_to_json(
    input_path="공동주택 분양가격의 산정 등에 관한 규칙(국토교통부령)(제01135호)(20220715).txt",
    output_path=None
):
    """
    공동주택 분양가격의 산정 등에 관한 규칙을 조 단위로 청킹하고 JSON 파일로 저장합니다.
    특징: 4개 장(Chapter), 20개 조항, 계산 공식 포함
    """
    ...
    rule_title = title_match.group(1).strip() if title_match else "공동주택 분양가격의 산정 등에 관한 규칙"
    ...
    for i, match in enumerate(article_matches):
        ...
        # 계산 공식 확인 (분양가격 산정 관련)
        has_formula = bool(re.search(r'[가-힣]+\s*[=＝]\s*[가-힣]+', article_content))
        ...
        # 분양가격 관련 조항인지 확인
        if "분양가격" in article_title or "산정" in article_title:
            metadata["is_pricing_related"] = True
    ...
    try:
        ...
        if chapter_article_count:
            print(f"\n장별 조항 분포:")
            for chapter, count in chapter_article_count.items():
        ...
        formula_count = sum(1 for chunk in chunks if chunk['metadata'].get('has_formula'))

        if appendix_count or form_count or law_count or decree_count:
            print(f"\n참조 통계:")
            if appendix_count:
            ...
            if formula_count:
                print(f"  계산 공식 포함: {formula_count}개 조항")
...
if __name__ == "__main__":
    chunk_apartment_pricing_rule_to_json()