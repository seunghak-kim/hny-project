...
def chunk_rental_housing_rule_to_json(
    input_path="민간임대주택에 관한 특별법 시행규칙(국토교통부령)(제01491호)(20250604).txt",
    output_path=None
):
    """
    민간임대주택에 관한 특별법 시행규칙을 조 단위로 청킹하고 JSON 파일로 저장합니다.
    특징: 장(Chapter) 구분이 없는 시행규칙, 52개 조항, 별표/별지 참조 많음
    """
    ...
    # 조항을 찾기 위한 정규표현식
    # 삭제되거나 이동된 조항도 포함 (제4조의 경우 제4조의5로 이동됨)
    ...
    for i, match in enumerate(article_matches):
        ...
        # 이동된 조항 처리 (예: 제4조는 제4조의5로 이동)
        if "→" in content[match.start():match.start()+50]:
            moved_match = re.search(r'→\s*(제\d+조(?:의\d+)?)', content[match.start():match.start()+50])
            moved_to = moved_match.group(1) if moved_match else ""

            chunk_id = "article_" + article_number.replace("제", "").replace("조", "").replace("의", "_")

            chunk_data = {
                "id": chunk_id,
                "text": f"{article_number} → {moved_to}로 이동",
                "metadata": {
        ...
        # 별표/별지 참조 추출
        form_references = re.findall(r'별[표지]\s*(?:제)?\d+(?:호)?(?:서식)?', article_content)
    ...
    try:
        ...
        moved_count = sum(1 for chunk in chunks if chunk['metadata'].get('is_moved', False))
        appendix_count = sum(1 for chunk in chunks if chunk['metadata'].get('in_appendix', False))
        active_count = len(chunks) - deleted_count - moved_count - appendix_count

        print(f"\n조항 통계:")
        print(f"  유효 조항: {active_count}개")
        if moved_count > 0:
            print(f"  이동 조항: {moved_count}개")
        if deleted_count > 0:
        ...
        # 별표/별지 참조 통계
        form_ref_count = sum(1 for chunk in chunks if chunk['metadata'].get('form_references'))
        if form_ref_count > 0:
            print(f"\n별표/별지 참조: {form_ref_count}개 조항에서 참조")
...
if __name__ == "__main__":
    # 실제 파일 처리
    chunk_rental_housing_rule_to_json()