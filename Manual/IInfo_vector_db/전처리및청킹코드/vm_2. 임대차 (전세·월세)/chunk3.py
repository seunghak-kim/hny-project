...
def chunk_rental_housing_law_to_json(
    input_path="민간임대주택에 관한 특별법(법률)(제20661호)(20250604).txt",
    output_path=None
):
    """
    민간임대주택에 관한 특별법을 조 단위로 청킹하고 JSON 파일로 저장합니다.
    특징: 7개 장으로 구성, 91개 조항 (가장 포괄적인 법률)
    """
    ...
    # 제목 및 메타데이터 추출
    title_match = re.search(r'^([^\n]+특별법)', content, re.MULTILINE)
    law_title = title_match.group(1).strip() if title_match else "특별법"
...
if __name__ == "__main__":
    # 실제 파일 처리
    chunk_rental_housing_law_to_json()