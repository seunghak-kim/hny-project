...
def chunk_housing_lease_law_to_json(
    input_path="주택임대차보호법(법률)(제19356호)(20230719).txt",
    output_path=None
):
    """
    주택임대차보호법을 조 단위로 청킹하고 JSON 파일로 저장합니다.
    특징: 장(Chapter) 구분 없음, 42개 조항, 제3조의2~7까지 연속된 하위조항, 임차인 보호 중심
    """
    ...
    # 제목 및 메타데이터 추출
    title_match = re.search(r'^([^\n]+보호법)', content, re.MULTILINE)
    law_title = title_match.group(1).strip() if title_match else "주택임대차보호법"
    ...
    for i, match in enumerate(article_matches):
        ...
        # 임차인 보호 관련 조항인지 확인
        if any(keyword in article_title or keyword in article_content[:100]
               for keyword in ["임차인", "보증금", "우선변제", "대항력", "임대차"]):
            metadata["is_tenant_protection"] = True

        # 제3조의2~7 연속 조항인지 확인
        if article_number.startswith("제3조의"):
            metadata["is_series_article"] = True
            metadata["series_group"] = "제3조"

        # 벌칙 관련 조항인지 확인
        if "벌칙" in article_title or "과태료" in article_title:
            metadata["is_penalty_related"] = True
    ...
    try:
        ...
        series_count = sum(1 for chunk in chunks if chunk['metadata'].get('is_series_article', False))
        tenant_protection_count = sum(1 for chunk in chunks if chunk['metadata'].get('is_tenant_protection', False))
        penalty_count = sum(1 for chunk in chunks if chunk['metadata'].get('is_penalty_related', False))
        ...
        if series_count > 0:
            print(f"  제3조 시리즈 조항: {series_count}개 (제3조의2~7)")
        if tenant_protection_count > 0:
            print(f"  임차인 보호 관련: {tenant_protection_count}개")
        if penalty_count > 0:
            print(f"  벌칙 관련: {penalty_count}개")
...
if __name__ == "__main__":
    # 실제 파일 처리
    chunk_housing_lease_law_to_json()