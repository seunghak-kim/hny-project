    ...
    async def execute_search_node(self, state: SearchTeamState) -> SearchTeamState:
        ...
        if "realestate_terminology" in selected_tools and self.terminology_tool:
            try:
                ...
                # 용어 검색 실행
                result = self.terminology_tool.search_terminology(query, top_k=5)
        ...
        if "building_registry" in selected_tools and self.building_registry_tool:
            try:
                ...
                if address_match:
                    if len(address_match.groups()) == 4:
                        # 시도 포함
                        address = f"{address_match.group(1)} {address_match.group(2)} {address_match.group(3)} {address_match.group(4)}"
                    else:
                        # 시도 생략
                        address = f"{address_match.group(1)} {address_match.group(2)} {address_match.group(3)}"

                    logger.info(f"[SearchTeam] Extracted address: {address}")
                    ...
                    state["search_progress"]["building_registry"] = "skipped"
                    execution_results["building_registry"] = {
                        "status": "skipped",
                        "reason": "No valid address found in query"
                    }
                    logger.info("[SearchTeam] No address pattern found in query, skipping building registry search")
