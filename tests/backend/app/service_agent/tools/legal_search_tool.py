...
class LegalSearch:
    ...
    def vector_search(
        ...
        try:
            ...
            # 쿼리 임베딩
            query_embedding = self.db_manager.embedding_model.encode(enhanced_query, convert_to_tensor=False)
        ...
        if chunk_ids:
            try:
                ...
                for chunk_id in chunk_ids:
                    meta = self.db_manager._faiss_meta_dict.get(chunk_id)
                    if meta:
    ...
    def get_law_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "faiss_vectors": self.db_manager.faiss_index.ntotal
        }
